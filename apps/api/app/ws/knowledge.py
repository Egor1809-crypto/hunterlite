...
async def _handle_hint(ws: WebSocket, state: _SoloQuizState) -> None:
    ...
    category_prompts = {
        "eligibility": (
            ...
            "Проверь три опоры: кто подаёт заявление, есть ли признаки неплатёжеспособности и выполнен ли порог по сумме/сроку.",
        ),
        "procedure": (
            "Процедура банкротства",
            "Раздели ответ по стадиям: принятие заявления, наблюдение/реализация, собрание кредиторов, итоговое судебное действие.",
        ),
        "property": (
            "Имущество должника",
            "Сначала отдели конкурсную массу от исключений, затем проверь залог, совместное имущество и исполнительский иммунитет.",
        ),
        "consequences": (
            "Последствия банкротства",
            "Смотри, что меняется после введения процедуры: ограничения, сделки, долги, статус должника и последствия завершения.",
        ),
        "costs": (
            "Расходы и вознаграждение",
            "Ищи, кто платит, когда вносятся средства и какие расходы относятся к обязательным судебным/процедурным.",
        ),
        "creditors": (
            "Кредиторы и реестр",
            "Определи очередь, срок включения в реестр и какие права даёт включённое требование.",
        ),
        "documents": (
            "Документы",
            "Подумай, какие документы подтверждают долг, имущество, доходы, сделки и состав кредиторов.",
        ),
        "timeline": (
            "Сроки",
            "Проверь процессуальный срок: когда он начинается, чем подтверждается и что будет при пропуске.",
        ),
        "court": (
            "Судебные действия",
            "Отталкивайся от компетенции арбитражного суда: какое заявление подаётся и какое определение должен вынести суд.",
        ),
        "rights": (
            "Права должника и участников",
            "Сравни права должника, кредитора и управляющего: кто вправе требовать, обжаловать, голосовать или получать сведения.",
        ),
    }

    category_key = (q.category or "general").strip().lower()
    category_name, category_hint = category_prompts.get(
        category_key,
        ("Общие положения ФЗ-127", "Сначала найди субъект, юридический факт и последствие: кто действует, что произошло и какая норма это регулирует."),
    )

    if tier == 1:
        text = f"Тема: {category_name}. {category_hint}"
    elif tier == 2:
        article = q.expected_article or "ФЗ-127"
        text = f"Нужная опора: {article}. Сверь факты вопроса с условиями нормы и не перескакивай сразу к выводу."
    else:
        ...
        try:
            ...
            text = f"Собери короткую формулу ответа: условие из нормы + факт из вопроса + вывод. Для этой темы ключевой ориентир — {category_name.lower()}."
