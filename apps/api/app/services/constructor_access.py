...
# ⚠ ГРАНИЦА ДОВЕРИЯ (ultrareview #1): источник истины — training_map_progress.test_map,
# который пишется КЛИЕНТОМ через PUT /training-map/progress (баллы теста считаются на
# фронте; серверного грейдинга квиза в кодбазе нет). Это сознательный выбор ТЗ §3
# (DECISION-4: «источник истины — test_map») и единый trust-model всего training-map
# (тесты/экзамены/энергия — все клиент-записываемые). Поэтому гейт — это UX-гейт
# ПРОГРЕССИИ (self-service разблокировка собственной фичи), а НЕ граница безопасности:
# подкованный пользователь может выставить test_map и открыть конструктор себе досрочно.
# Межпользовательского доступа/эскалации/выгоды тут нет. Жёсткий вариант (серверный
# грейдинг региона 1 в отдельной таблице попыток) — отдельная задача вне CONSTRUCTOR_TZ.
