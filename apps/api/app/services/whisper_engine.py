...
# 2026-06-04: hints reframed sales→legal consultation (keys kept).
SCRIPT_STUCK_HINTS: dict[str, str] = {
    "greeting": "Затянули представление. Коротко: «Здравствуйте, я {имя}, юрист по банкротству физлиц. Помогу разобраться с долгами.»",
    "contact": "Застряли на контакте. «Как к вам обращаться? Расскажите, что у вас за ситуация — я выслушаю.»",
    "qualification": "Выяснение буксует. «Какая сумма долга? Есть ли ипотека, иждивенцы, сделки за последние 3 года?»",
    "presentation": "Правовой анализ затянулся. Проще: «По ФЗ-127 два пути — реструктуризация или реализация; единственное жильё под защитой ст. 446 ГПК.»",
    "objections": "Страхи висят. Присоединитесь: «Понимаю, страшно за квартиру. Единственное жильё защищено законом — без ложных гарантий списания.»",
    "appointment": "Пора дать рекомендацию: «По вашей ситуации подойдёт <путь>; следующий шаг — собрать документы.»",
    "closing": "Подведите итог: «Итак, путь — <X>, дальше готовим заявление и список кредиторов. Спасибо за доверие.»",
}
...
STAGE_HINTS: dict[str, str] = {
    "greeting": "Представьтесь как юрист по банкротству физлиц, обозначьте цель — помочь разобраться с долгами. Коротко.",
    "contact": "Установите контакт: обратитесь по имени, проявите эмпатию к должнику в стрессе, снизьте тревожность.",
    "qualification": "Выясните обстоятельства: сумму долга, кредиторов, доход, иждивенцев, жильё/ипотеку, сделки за 3 года, стадию взыскания.",
    "presentation": "Дайте правовой анализ по ФЗ-127: реструктуризация vs реализация, единственное жильё (ст. 446 ГПК), сроки.",
    "objections": "Снимите страхи и сомнения («заберут квартиру», «всё спишут») — без ложных гарантий списания.",
    "appointment": "Дайте рекомендацию: верный путь по ситуации + конкретный следующий шаг.",
    "closing": "Подведите итог: путь, документы, следующий шаг. Поблагодарите, корректно завершите.",
}
