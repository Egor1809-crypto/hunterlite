...
class QuizQuestion:
    ...
    # PR-MC (2026-05-05): multiple-choice format. When `choices` is populated,
    # the FE renders 3 buttons instead of a textarea and the WS answer
    # handler accepts `choice_index` instead of free text. `correct_choice_index`
    # is populated server-side and NEVER serialized to the client (see
    # ws/knowledge.py answer routing). Distractor generation flows through
    # _enrich_with_choices() — LLM generates 2 plausible-but-wrong options
    # paired with the RAG-grounded correct one, then caches them on the
    # source LegalKnowledgeChunk.choices column for reuse.
...
AI_EXAMINER_PROMPT = """Ты — СТРОГИЙ, но доброжелательный AI-наставник по Федеральному Закону №127-ФЗ «О несостоятельности (банкротстве)».

Твоя личность:
- Спокойный, ясный, по делу — без театральности и шуток
- Хвалишь за правильные ответы коротко и по существу
- При неправильных ответах — не ругаешь, спокойно объясняешь правильный вариант
- ОБРАЩЕНИЕ (важно): всегда обращайся к пользователю напрямую на «Вы»; если в контексте указано его имя — обращайся по имени. НИКОГДА не пиши о нём в третьем лице («студент», «ученик», «пользователь перепутал/ошибся») — только «Вы …».
- Обратная связь КРАТКАЯ: 2–3 предложения, без воды (это и быстрее для пользователя).
...
# ---------------------------------------------------------------------------
# Multiple-choice enricher (PR-MC, 2026-05-05)
# ---------------------------------------------------------------------------
#
# User asked for "формат теста — AI генерит 3 варианта, один из РАГ".
# This module enriches an already-built QuizQuestion with a 3-option
# multiple-choice payload:
#   - 1 correct option from RAG (correct_response_hint or blitz_answer)
#   - 2 plausible-but-wrong distractors generated by the LLM
#
# Caching: the resulting (correct + distractors) tuple is persisted on
# LegalKnowledgeChunk.choices/correct_choice_index, so subsequent calls
# for the same chunk skip the LLM round-trip. The `choices` column was
# already declared (models/rag.py:98) — this PR is the first writer.
# ---------------------------------------------------------------------------
...
async def _generate_mc_distractors(
    *,
    question_text: str,
    correct_answer: str,
    ...
    """LLM call: build 2 plausible-but-wrong options for a given question."""
    user_prompt = (
        f"Вопрос: {question_text}\n"
        f"Правильный ответ: {correct_answer}\n"
        + (f"Статья закона: {article}\n" if article else "")
        + "\nВерни 2 неправильных варианта в JSON."
    )
    try:
        response = await asyncio.wait_for(
            generate_response(
                system_prompt=_MC_DISTRACTOR_PROMPT,
                messages=[{"role": "user", "content": user_prompt}],
                user_id=user_id or "mc-distractor",
                task_type="structured",
        ...
        # Sanity: must be 2 strings, non-empty, distinct from correct
        cleaned = [
...
async def enrich_question_with_choices(
    ...
    *,
    ...
    """Populate choices + correct_choice_index on a freshly-generated question.

    Order of operations:
      1. If the source chunk already has cached choices in DB → use them.
      2. Otherwise, derive correct answer from blitz_answer or
         correct_response_hint or first RAG result.
      3. Call LLM to generate 2 distractors.
      4. Shuffle the 3 options, record the correct index.
      5. Cache the result on the LegalKnowledgeChunk row so future
         questions on the same chunk skip the LLM round-trip.

    Returns the same QuizQuestion with `choices` and `correct_choice_index`
    populated, OR returns the question unchanged if no correct answer
    could be derived (caller falls back to free-text mode).
    """
    ...
    if len(correct_text) > 220:
        correct_text = correct_text[:217].rsplit(" ", 1)[0] + "…"

    # 3. LLM distractors
    distractors = await _generate_mc_distractors(
    ...
    question.correct_choice_index = correct_idx

    # 5. Cache on the chunk row (best-effort, non-blocking on failure)
    if chunk is not None:
        try:
            chunk.choices = shuffled
            chunk.correct_choice_index = correct_idx
            db.add(chunk)
            await db.commit()
        except Exception:
            logger.warning("MC choice cache write failed for chunk %s", question.chunk_id, exc_info=True)
            await db.rollback()
