...
async def unified_history(
    ...
    # Generous per-kind bound: keeps the merged set bounded while preserving
    # correct ordering for realistic pages (the real cap is the merge+slice).
    _bound = offset + limit + 200
    ...
    ts_result = await db.execute(
        ...
        ).order_by(TrainingSession.started_at.desc()).limit(_bound)
    )
    ...
    ca_result = await db.execute(
        ...
        ).order_by(CaseAttempt.finished_at.desc()).limit(_bound)
    )
    ...
    ea_result = await db.execute(
        ...
        ).order_by(ExamAttempt.finished_at.desc()).limit(_bound)
    )
    ...
    qz_result = await db.execute(
        ...
        ).order_by(KnowledgeQuizSession.ended_at.desc()).limit(_bound)
    )
...
async def _weak_points_exam(attempt: ExamAttempt, db: AsyncSession) -> list[str]:
    """TZ-4 exams are ITEM-based: derive weak points from missed ExamItems
    (ExamItemAttempt.passed is False), bucketed by article_reference."""
    weak: list[str] = []
    result = await db.execute(
        select(ExamItem)
        .join(ExamItemAttempt, ExamItemAttempt.item_id == ExamItem.id)
        .where(
            ExamItemAttempt.attempt_id == attempt.id,
            ExamItemAttempt.passed.is_(False),
        )
    )
    cats: dict[str, int] = {}
    for item in result.scalars().all():
        bucket = (item.article_reference or "").strip() or (item.type or "").strip()
        if not bucket:
            bucket = "Без привязки к статье"
...
async def explain(
    ...
    if kind == "exam":
        ...
        try:
            quiz_uuid = uuid.UUID(item_id)
        except ValueError:
            raise HTTPException(status_code=404, detail="Quiz session not found")
        try:
            q = (await db.execute(
                select(KnowledgeQuizSession).where(
                    KnowledgeQuizSession.id == quiz_uuid,
                    KnowledgeQuizSession.user_id == user.id,
                )
            )).scalars().first()
        except Exception:
            logger.exception("Manyasha explain: quiz lookup failed")
            q = None
        if q is None:
            raise HTTPException(status_code=404, detail="Quiz session not found")
        ...
        try:
            if kind == "story":
                sess = list((await db.execute(
                    select(TrainingSession).where(
                        TrainingSession.user_id == user.id,
                        TrainingSession.client_story_id == parsed_id,
                        TrainingSession.status == SessionStatus.completed,
                    )
                )).scalars().all())
            else:
                sess = list((await db.execute(
                    select(TrainingSession).where(
                        TrainingSession.user_id == user.id,
                        TrainingSession.id == parsed_id,
                        TrainingSession.status == SessionStatus.completed,
                    )
                )).scalars().all())
        except Exception:
            logger.exception("Manyasha explain: training session lookup failed")
            sess = []
        if not sess:
            raise HTTPException(status_code=404, detail="Training session not found")
