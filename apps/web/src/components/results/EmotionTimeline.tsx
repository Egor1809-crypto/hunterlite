...
export default function EmotionTimeline({ timeline, journeySummary, onReplayMessage }: Props) {
  ...
  if (journeySummary) {
    ...
    if (peakValue < 40) {
      tips.push({
        tone: "warn",
        title: "Клиент не вышел из защиты",
        body: "Пик разговора остановился на защитной эмоции. Раньше выходите на тревогу должника и опирайтесь на конкретику ситуации.",
      });
    } else if (peakValue >= 60 && peakValue < 95) {
      tips.push({
        tone: "info",
        title: "Клиент обдумывает, но не готов действовать",
        body: "Был близок к решению — не хватило финального шага. Чётче формулируйте рекомендацию и сразу обозначайте следующий шаг.",
      });
    } else if (peakValue >= 95) {
      tips.push({
        tone: "good",
        title: "Клиент согласился на консультацию — хорошо",
        body: "Удерживайте этот паттерн: раннее снятие тревоги + конкретика по ФЗ-127 + ясная рекомендация и следующий шаг.",
      });
    }
