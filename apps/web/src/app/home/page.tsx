...
export default function HomePage() {
  ...
  const [learningPath, setLearningPath] = useState<LearningPathProgress | null>(
    null,
  );
  const [recommendations, setRecommendations] = useState<Recommendation[]>([]);
  const [waitingClient, setWaitingClient] = useState<WaitingClient | null>(
    null,
  );
  ...
  const fetchData = useCallback(() => {
    ...
    api
      .get<LearningPathProgress>("/learning-path/progress")
      .then(setLearningPath)
      .catch((err) => logger.error("Failed to load learning path:", err))
      .finally(() => setLoading(false));

    api
      .get<{ client: WaitingClient | null }>("/home/waiting-client")
  ...
  const quickStart = async () => {
    ...
    try {
      ...
      const scenarios: { id: string }[] = Array.isArray(scenariosData)
        ? scenariosData
        : [];
      if (!scenarios.length) {
  ...
  const activeStageIndex = stages.findIndex(
    (stage) => (progress[stage.key] ?? 0) < 100,
  );
  const activeStage =
    activeStageIndex === -1 ? stages.length - 1 : activeStageIndex;
      ...
      >
        <div className="mx-auto max-w-[920px]">
          ...
          >
            <div>
              <h1
                className="max-w-3xl font-display font-semibold leading-[0.95] tracking-[-0.045em]"
                style={{
              ...
              <p
                className="mt-5 max-w-xl text-lg leading-snug"
            ...
            <Card variant="editorial" accentTop className="lg:self-stretch">
              <p
          ...
          <section className="mb-12">
            <SectionHeader
              ...
              right={
                <span
                  className="hidden font-mono text-[12px] font-semibold tabular-nums sm:inline-flex"
                  style={{ color: "var(--text-muted)" }}
                >
                  ШАГ {activeStage + 1}/5
                </span>
              ...
              {stages.map((stage, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <h3>{stage.title}</h3>
                    <span className="editorial-index-status">
                      {value >= 100 ? "Пройдено" : `${value}%`}
                    </span>
                    <ArrowRight size={20} />
                  </Link>
                );
          ...
          {/* ── Действия ── */}
          <section className="mb-12 grid gap-x-8 lg:grid-cols-3">
            <ActionCard
              eyebrow="Практика"
              title={waitingClient ? "Входящий клиент" : "Тренировочный клиент"}
              text={
                waitingClient
                  ? `${waitingClient.full_name}, ${waitingClient.city}. Долг ${(waitingClient.total_debt / 1000).toFixed(0)} тыс. ₽`
                  : "Короткая консультация: входящий должник."
              }
          ...
          <section className="grid gap-4 lg:grid-cols-[1fr_360px]">
            ...
            <Card variant="editorial" accentTop className="flex flex-col">
              <p
                className="font-mono text-[11px] font-semibold uppercase tracking-[0.16em]"
                style={{ color: "var(--text-secondary)" }}
              >
              ...
              <p
                className="mt-3 flex-1 text-[15px] leading-snug"
                style={{ color: "var(--text-secondary)" }}
              >
                {primaryRecommendation?.description ||
                  "Закрепите материал тестом или коротким кейсом."}
              </p>
              <Button
...
function ActionCard({
  ...
  if (href) {
    return (
      <Link href={href} className="group block no-underline">
        <Card variant="editorial" style={{ height: "100%" }}>
          {inner}
        </Card>
      </Link>
    );
  ...
  return (
    <button
      className="group w-full text-left py-6 border-b"
      onClick={onClick}
      disabled={disabled}
      style={{
        height: "100%",
...
function MetricCard({
  ...
  return (
    <Card variant="editorial">
      ...
      <div
        className="mt-7 font-mono text-[44px] font-semibold leading-none tabular-nums"
        style={{ color: "var(--text-primary)" }}
      >
        {loading ? (
          <Skeleton width={64} height={40} rounded="8px" />
        ) : (
          <>
            {value}
            {suffix ?? ""}
