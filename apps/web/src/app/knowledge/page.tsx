...
const QUICK_TOPICS = [
  {
    ...
    icon: ClipboardList,
    description: "Наблюдение, реструктуризация, конкурсное производство",
    category: "procedure",
  },
  {
    title: "Оспаривание сделок",
    icon: Scale,
    description: "Ст. 61.2, ст. 61.3 — подозрительные и преференциальные",
    category: "property",
  },
  {
    title: "Субсидиарная ответственность",
    icon: Target,
    description: "КДЛ, доказывание, размер ответственности",
    category: "consequences",
  },
  {
    title: "Торги и реализация",
    icon: Gavel,
    description: "Порядок проведения, оспаривание результатов",
    category: "costs",
  },
  {
    title: "Банкротство физлиц",
    icon: User,
    description: "Процедура, ограничения, списание долгов",
    category: "eligibility",
  },
  {
    title: "Работа с кредиторами",
    icon: Users,
    description: "Реестр, собрания, голосование, очерёдность",
    category: "creditors",
  },
];
...
function impactMeta(level: "high" | "medium" | "low"): {
  label: string;
  color: string;
} {
...
export default function KnowledgePage() {
  const [activeSection, setActiveSection] = useState<"browse" | "radar" | "ai">(
    "browse",
  );
  ...
  useEffect(() => {
    api
      .get<KnowledgeStats>("/knowledge-ai/stats")
      .then(setStats)
      .catch(() => {});
  }, []);
  ...
  const loadRadar = useCallback(async (cat?: string | null) => {
    ...
    try {
      ...
      const data = await api.get<RadarResponse>(
        `/knowledge-ai/radar?${params}`,
      );
  ...
  const loadRadarCategories = useCallback(async () => {
    try {
      const cats = await api.get<{ category: string; count: number }[]>(
        "/knowledge-ai/radar/categories",
      );
        ...
        <div className="relative z-10 mx-auto max-w-[920px] px-5 py-8 sm:px-8 sm:py-12">
          ...
          <div className="mb-10 grid grid-cols-3 gap-3 sm:gap-4">
            {statsDisplay.map((stat) => (
              <Card variant="editorial" key={stat.label}>
                <div
                ...
                <div
                  className="mt-3 font-mono text-2xl font-semibold tabular-nums tracking-tight sm:text-3xl"
          ...
          <AnimatePresence mode="wait">
            ...
            {activeSection === "browse" && (
              ...
              >
                ...
                <div className="editorial-topics mb-10">
            ...
            {activeSection === "radar" && (
              ...
              >
                {/* Radar header */}
                <Card variant="editorial" accentTop>
                  <div className="flex items-start gap-4">
                    <div className="flex-1 min-w-0">
                      ...
                      <p
                        ...
                        style={{ color: "var(--text-secondary)" }}
                      >
                        Изменения в ФЗ-127, разъяснения ВС РФ и практика
                        арбитражных судов.
                      </p>
                      {radarLastUpdated && (
                        <div
                          ...
                          style={{ color: "var(--text-muted)" }}
                        >
                          <Clock size={11} /> Обновлено{" "}
                          {formatRelativeTime(radarLastUpdated)}
                        </div>
                      )}
                    </div>
                    <Button
                ...
                {radarCategories.length > 0 && (
                  <div className="flex flex-wrap gap-2">
                    ...
                    {radarCategories.map((cat) => (
                      <FilterChip
                        key={cat.category}
                        active={radarCategory === cat.category}
                ...
                {radarLoading && radarItems.length === 0 && (
                  <div className="space-y-3">
                    {[1, 2, 3].map((i) => (
                      <Card variant="editorial" key={i}>
                        <div
                          className="h-3 w-3/4 animate-pulse rounded"
                          style={{ background: "var(--bg-tertiary)" }}
                        />
                        <div
                          className="mt-3 h-2 w-1/2 animate-pulse rounded"
                          style={{ background: "var(--bg-tertiary)" }}
                        />
                      </Card>
                    ))}
                ...
                {/* Items */}
                {radarItems.map((item) => {
                  const impact = impactMeta(
                    getImpactLevel(item.relevance_score),
                  );
                ...
                {/* Empty */}
                {!radarLoading && radarItems.length === 0 && (
                  <Card variant="editorial" padded={false}>
                    <EmptyState
                      icon={Bell}
                      title="Пока тихо"
