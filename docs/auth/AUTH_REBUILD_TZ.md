...
## 3. SMS-вход по номеру телефона — ❌ СНЯТО (решение владельца 2026-06-19)

> **НЕ реализовывать.** Владелец: «SMS-провайдер пока вообще не надо». Yandex ID +
> собственный email/пароль перекрывают требование 149-ФЗ (минимум один разрешённый
> метод). Раздел сохранён на случай возврата к задаче; в коде SMS не начат
> (миграции `phone` нет, что и требуется).
