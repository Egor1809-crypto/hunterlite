...
## 11. Решения (зафиксированы заказчиком 2026-06-02)
- **DECISION-A → новые типы в `exam_item`**, MCQ-банк `ExamQuestion` как мост (постепенно поглотить).
- **DECISION-B → эталон = exam-3 «Анализ дела»** (на нём виден весь AI-грейд-конвейер).
- **DECISION-C → AI-грейд синхронно** в submit для эталона; async-очередь — если генерация долгая.
- **DECISION-D → per-exam пороги задать на этапе контента** (убрать хардкод flat 88).
